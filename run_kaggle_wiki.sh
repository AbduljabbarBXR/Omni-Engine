...
print("wikitext2 ready", len(ds["train"]["text"]), "train articles", len(ds["validation"]["text"]), "valid articles")
EOF
python run_ablation.py --steps 2000 --base EleutherAI/pythia-160m-deduped --delta-penalty 0.1 --out runs/wiki_ablation --data data/wikitext2_train.txt --eval-data data/wikitext2_valid.txt
